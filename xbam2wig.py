...
##This script can also be used for calibrated ChIp-seq 

## 0.1.5 correct the normalization factor 2017-02-23
        ...
        if norm:
            for chrs in self.chrs:
                ...
                for bin_id,value in enumerate(self.bins[self.chr2tid[chrs]]):
                    ...
                    self.bins[self.chr2tid[chrs]][bin_id] = (self.bins[self.chr2tid[chrs]][bin_id] * 1.0 )/args.norm
...
def parse_argument():
    ...
    # Binsize
    p.add_argument("-b","--binsize",dest="binsize",type=int,metavar="Binsize",default=50,
    ...
    # the chip experiment
    p.add_argument("--bam",dest="bam",type=str,metavar='Bam',required = True, 
                   help="The ChIP experiment bam file")
    p.add_argument('-n', "--norm", dest='norm', type=float, metavar="Norm", default=1, help="The library size. Default: Normalized to 10 million mapped reads, If you performed Calibrated ChIP-seq, you can set it to the total mapped spike-in library size (for example, if you used spike-in chromatin and got 1500000, you can set it 0.15)")
...
def main():
    ...
    print >>sys.stderr,"# xbam2wig.py 0.1.5 "
